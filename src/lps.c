...
// if prefix length byte <= 7F then it is str length
...
lps lps_cat(lps s1, const lps s2) {
	...
	lps out = lps_with_reserved_len(len1 + len2);
	memcpy(out, s1, len1);
...
lps lps_from_chars_with_len(const char *s, const size_t len) {
	lps new = lps_with_reserved_len(len);
	memcpy(new, s, len);
	return new;
}
...
lps lps_from_cstr(const char *s) {
	...
	return lps_from_chars_with_len(s, len);
}
...
#define LPS_TEST_INNER(str, id) \
	const char *CONCAT2(_cstr_, id) = str; \
	lps CONCAT2(lps_str_, id) = lps_from_cstr(CONCAT2(_cstr_, id)); \
	printf("lps: "LPS_Fmt"\n", LPS_Arg(CONCAT2(lps_str_, id))); \
	assert(memcmp(CONCAT2(lps_str_, id), str, strlen(str)) == 0); \
	printf("len: %zu\n", lps_len(CONCAT2(lps_str_, id))); \
	assert(strlen(str) == lps_len(CONCAT2(lps_str_, id))); \
	lps_free(CONCAT2(lps_str_, id));
